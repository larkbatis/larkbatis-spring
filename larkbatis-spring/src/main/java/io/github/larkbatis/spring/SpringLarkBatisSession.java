 ...
 */
public final class SpringLarkBatisSession implements LarkBatisSession {

    /** The {@code task} label Spring puts in front of every translated message. */
    private static final String TASK = "LarkBatis";
     ...
     * table.
     */
    public SpringLarkBatisSession(DataSource dataSource) {
        this(dataSource, new SQLExceptionSubclassTranslator());
    }

    public SpringLarkBatisSession(DataSource dataSource, SQLExceptionTranslator translator) {
    ...
    /**
     * Into Spring's {@code DataAccessException} hierarchy, so a LarkBatis
     ...
     * LarkBatis exception is the floor under that case, never a raw
    ...
    public RuntimeException translate(SQLException e, String sql) {
        ...
        return new LarkBatisException(TASK + " failed: " + e.getMessage()
