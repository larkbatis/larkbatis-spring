...
2. **The generated `@Configuration` replaces `@MapperScan`.**
   ...
   `@ComponentScan` finds `LightBatisMapperConfiguration` in this very package
   — a file the processor emitted — and the Boot auto-configuration supplies
   the one `LightBatisSession` it asks for.
3. **`conn()` joins whatever transaction is running.** LightBatis never manages
...
The `lightbatis.*` block is there to prove the properties bind — this sample
has no `${}` at all, so neither setting does any work.

## How to run

```bash
./gradlew :lightbatis-spring-sample:test    # SpringIntegrationTest
./gradlew :lightbatis-spring-sample:build   # compile + test + jar
```
...
`AccountMapper$$Impl.java`, `AccountRow.java`, `LightBatisMappers.java` and
`LightBatisMapperConfiguration.java`.

## Cross-repo builds

`settings.gradle.kts` at the repo root `includeBuild("../lightbatis")` when
that directory exists, so a local checkout of the core repo is substituted for
the published `io.github.lightbatis:*` coordinates. It is conditional because
