 ...
 * only honest answer.
 */
class LarkBatisAutoConfigurationTest {

    private final ApplicationContextRunner runner = new ApplicationContextRunner()
            .withConfiguration(AutoConfigurations.of(LarkBatisAutoConfiguration.class));
     ...
     * application asks for a {@code LarkBatisSession} and gets
    ...
    void ordersItselfAfterDataSourceOnBothBootGenerations() {
        AutoConfiguration annotation =
                LarkBatisAutoConfiguration.class.getAnnotation(AutoConfiguration.class);
    ...
    void contributesASessionForASingleDataSource() {
        runner.withUserConfiguration(OneDataSource.class).run(context -> {
            assertThat(context).hasSingleBean(LarkBatisSession.class);
            assertThat(context.getBean(LarkBatisSession.class))
                    .isInstanceOf(SpringLarkBatisSession.class);
    ...
    void backsOffWithoutADataSource() {
        runner.run(context -> assertThat(context).doesNotHaveBean(LarkBatisSession.class));
    }

    @Test
    void backsOffWithTwoDataSourcesAndNoPrimary() {
        runner.withUserConfiguration(TwoDataSources.class).run(context -> {
            // the application declares one SpringLarkBatisSession per DataSource
            // itself; guessing here would wire mappers to the wrong database
            assertThat(context).doesNotHaveBean(LarkBatisSession.class);
    ...
    void appliesTheVariantProperties() {
        runner.withUserConfiguration(OneDataSource.class)
                .withPropertyValues("larkbatis.max-sql-variants=7",
                        "larkbatis.fail-on-unbounded-fragment=true")
                .run(context -> {
                    LarkBatisProperties properties = context.getBean(LarkBatisProperties.class);
    ...
    static class OwnSession {
        @Bean
        LarkBatisSession mySession(DataSource dataSource) {
            return new SpringLarkBatisSession(dataSource);
        }
    }
