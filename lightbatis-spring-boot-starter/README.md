...
Published: `io.github.lightbatis:lightbatis-spring-boot-starter`.

## What it pulls in

| Coordinate | Why |
|---|---|
| `lightbatis-runtime` | What generated mapper bodies call |
| `lightbatis-spring` | `SpringLightBatisSession` |
| `lightbatis-spring-boot-autoconfigure` | The two beans and the `lightbatis.*` properties |
...
`SqlSessionTemplate`. The processor emits `LightBatisMapperConfiguration` into
...
auto-configuration supplies the single `LightBatisSession` those `@Bean`
methods ask for.

`lightbatis-annotations` and `lightbatis-processor` are **not** here on
...
`io.github.lightbatis.spring` instead.
...
```bash
./gradlew :lightbatis-spring-boot-starter:build
```
